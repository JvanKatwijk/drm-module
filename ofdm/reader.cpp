...
//	The methods are called from the drmdecoder
	...
	Reader::Reader (RingBuffer<Complex> *r,
	                uint32_t bufSize, RadioInterface *m_form) {
	ringBuffer		= r;
	(void)bufSize;
	this -> bufSize		= 16 * 8192;
	this	-> bufMask	= this -> bufSize - 1;
	data			= new Complex [this -> bufSize + 1];
	this	-> m_form	= m_form;
	memset (data, 0, this -> bufSize * sizeof (Complex));
	currentIndex		= 0;
	firstFreeCell		= 0;
	stopSignal		= false;
