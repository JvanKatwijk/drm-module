...
//	for detecting pilots:
	...
	this	-> symbolBuffer	= new Complex *[nrSymbols];
	for (int i = 0; i < nrSymbols; i ++)
	   symbolBuffer [i] = new Complex [Tu];
...
float	square (Complex v) {
	return real (v * conj (v));
}

int32_t	freqSyncer::get_zeroBin (int16_t start) {
Complex* correlationSum = dynVec (Complex, Tu);
float	*abs_sum = dynVec (float, Tu);
float	*squares = dynVec (float, Tu);
//
...
//	accumulate phase diffs of all carriers in subsequent symbols
	for (int j = start + 1; j < start + nrSymbols - 1; j++) {
	   ...
	   for (int i = 0; i < Tu; i++) {
	      Complex tmp1 = symbolBuffer [jmin1][i] *
...
int16_t	freqSyncer::getWord (Complex	*buffer,
	                     int32_t	bufSize,
	                     int32_t	theIndex,
	                     int16_t	wordNumber,
	                     int	intOffset,
...
//	we do some interpolation between samples in the time domain
	...
	for (int i = 0; i < Ts; i ++) {
	   Complex een = buffer  [(f + i) & bufMask];
	   Complex twee = buffer [(f + i + 1) & bufMask];
	   temp [i] = een * (float)(1 - offsetFractional) +
	              twee * (float)(offsetFractional);
	}
...
//	Fft_transform (&temp[Tg], Tu, false);
	memcpy (symbolBuffer [wordNumber],
	        &temp [Tg + Tu / 2], Tu / 2 * sizeof (Complex));
	memcpy (&symbolBuffer [wordNumber] [Tu / 2],
	        &temp [Tg] , Tu / 2 * sizeof (Complex));
	return 0;
}
