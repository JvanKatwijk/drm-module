...
void	aacProcessor_fdk::toOutput (Complex *b, int16_t cnt) {
	if (cnt == 0)
	   return;
	
	audioOut        -> putDataIntoBuffer (b, cnt);
	if (audioOut -> GetRingBufferReadAvailable () > 2400)
	   audioAvailable ();
}
...
void	aacProcessor_fdk::writeOut (int16_t *buffer, int16_t cnt,
	...
	Complex *local =
	          (Complex *)alloca (bufferSize * sizeof (std::complex<float>));
	
	for (int i = 0; i < cnt / 2; i ++) {
	   Complex tmp = Complex (buffer [2 * i] / 32767.0,
...
void	aacProcessor_fdk::reinit (std::vector<uint8_t> newConfig,
	...
	for (uint16_t i = 0; i < newConfig. size (); i ++) {
