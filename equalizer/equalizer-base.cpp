...
int16_t	i;
	...
	init_gain_ref_cells ();
	this	-> refFrame	= new Complex *[symbolsinFrame];
	for (i = 0; i < symbolsinFrame; i ++)
	   refFrame [i] = new Complex [carriersinSymbol];
	this	-> testFrame	= new Complex *[symbolsinFrame];
	for (i = 0; i < symbolsinFrame; i ++)
	   testFrame [i] = new Complex [carriersinSymbol];
}
...
void	equalizer_base::init_gain_ref_cells () {
...
float	equalizer_base::getMeanEnergy	() {
	return meanEnergy;
}

Complex	**equalizer_base::getChannels	() {
	return refFrame;
}
//
//
bool	equalizer_base::equalize (Complex *testRow,
	                          int16_t index,
